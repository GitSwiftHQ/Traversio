...
typedef struct {
    UINT8 key[UMAC_KEY_LEN];
} traversio_aes_key;
typedef traversio_aes_key aes_int_key[1];

static void
traversio_aes_encrypt_block(const UINT8 *in, UINT8 *out, aes_int_key int_key)
{
    size_t out_len = 0;
    ...
    if (status != kCCSuccess || out_len != AES_BLOCK_LEN) {
        explicit_bzero(out, AES_BLOCK_LEN);
    }
}

static void
traversio_aes_key_setup(const UINT8 *key, aes_int_key int_key)
{
    memcpy(int_key[0].key, key, UMAC_KEY_LEN);
}

#define aes_encryption(in,out,int_key) \
  traversio_aes_encrypt_block((const UINT8 *)(in), (UINT8 *)(out), (int_key))
#define aes_key_setup(key,int_key) \
  traversio_aes_key_setup((const UINT8 *)(key), (int_key))
